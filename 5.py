...
ROW_STYLES = {
    ("Sale", "Standard"):       {"bg": "#FADBD8", "fg": "#8B0000", "font": ("Arial", 10, "bold")},
    ("Sale", "Zero-rated"):     {"bg": "#F5B7B1", "fg": "#641E16", "font": ("Arial", 10, "normal")},
    ("Sale", "Exempt"):         {"bg": "#EBDEF0", "fg": "#4A235A", "font": ("Arial", 10, "italic")},
    ("Purchase", "Standard"):   {"bg": "#F4CCCC", "fg": "#660000", "font": ("Arial", 10, "bold")},
    ("Purchase", "Zero-rated"): {"bg": "#EA9999", "fg": "#5C0000", "font": ("Arial", 10, "normal")},
    ("Purchase", "Exempt"):     {"bg": "#F0F0F0", "fg": "#424242", "font": ("Arial", 10, "italic")},
}
...
class LoginWindow:
    ...
    def build_ui(self):
        ...
        self.error_label = tk.Label(card, text="", bg="white", fg="#000000",
        ...
        tk.Label(card, text=cred_info,
                 bg="white", font=("Arial", 7), fg="#000000", wraplength=280,
        ...
        style.configure('TButton', background='#8B0000', foreground='white')
...
class VATApp:
    ...
    def create_header(self):
        ...
        title = tk.Label(
            ...
            bg="#8B0000", fg="white", font=("Arial", 22, "bold")
        )
        title.pack(side="left", padx=25, pady=15)

        right_frame = tk.Frame(header, bg="#8B0000")
        ...
        tk.Label(
            ...
            bg="#8B0000", fg="#FFB6C1", font=("Arial", 9)
        ).pack(side="top", anchor="e")
    ...
    def create_dashboard(self):
        outer = tk.Frame(self.root, bg="#F5EFEF")
        outer.pack(fill="x", padx=20, pady=15)

        header_row = tk.Frame(outer, bg="#F5EFEF")
        header_row.pack(fill="x", pady=(0, 8))
        tk.Label(
            header_row, text="VAT DASHBOARD", bg="#F5EFEF",
            font=("Arial", 12, "bold"), fg="#8B0000"
        ).pack(side="left")
        self.dashboard_period_label = tk.Label(
            header_row, text="", bg="#F5EFEF", font=("Arial", 9), fg="#777777"
        )
        self.dashboard_period_label.pack(side="right")

        cards_row = tk.Frame(outer, bg="#F5EFEF")
        cards_row.pack(fill="x")

        self.output_card = self.create_card(cards_row, "OUTPUT VAT", "#C41C3B", "$0.00", "on sales")
        self.output_card.pack(side="left", fill="both", expand=True, padx=6)

        self.input_card = self.create_card(cards_row, "ALLOWABLE INPUT VAT", "#A4161A", "$0.00", "on purchases")
        self.input_card.pack(side="left", fill="both", expand=True, padx=6)

        self.net_card = self.create_card(cards_row, "NET VAT", "#DC143C", "$0.00", "payable")
        self.net_card.pack(side="left", fill="both", expand=True, padx=6)

        self.count_card = self.create_card(cards_row, "TRANSACTIONS", "#8B0000", "0", "recorded")
        self.count_card.pack(side="left", fill="both", expand=True, padx=6)

        self.rate_card = self.create_card(cards_row, "EFFECTIVE VAT RATE", "#E63946", "0.0%", "of taxable sales")
    ...
    def create_card(self, parent, title, accent, value, subtitle):
        outer = tk.Frame(parent, bg="#F5EFEF")
        canvas = tk.Canvas(outer, height=140, bg="#F5EFEF", highlightthickness=0)
    ...
    def create_legend(self):
        frame = tk.Frame(self.root, bg="#F5EFEF")
        frame.pack(fill="x", padx=20, pady=(0, 4))

        tk.Label(frame, text="Legend:", bg="#F5EFEF", font=("Arial", 9, "bold")).pack(side="left", padx=(0, 8))

        for (ttype, category), style in ROW_STYLES.items():
            swatch = tk.Frame(frame, bg="#F5EFEF")
            ...
            tk.Label(
                swatch, text=f" {ttype} / {category}", bg="#F5EFEF", fg=style["fg"], font=legend_font
            ).pack(side="left")
    ...
    def create_totals_bar(self):
        frame = tk.Frame(self.root, bg="#F9E6E6")
        frame.pack(fill="x", padx=20)
        self.totals_label = tk.Label(
            frame, text="", bg="#F9E6E6", font=("Arial", 9, "bold"), anchor="w"
        )
    ...
    def show_credentials_info(self):
        ...
        tk.Label(info_frame, text="Built-in Credentials (Stored in Code):", 
                font=("Arial", 11, "bold"), fg="#8B0000").pack(anchor="w", pady=(10, 5))
        ...
        tk.Label(info_frame, text="File-based Credentials (User Registered):", 
                font=("Arial", 11, "bold"), fg="#8B0000").pack(anchor="w", pady=(15, 5))
    ...
    # ========================================================
    # ADMIN PANEL (Access Control & User Records)
    # ========================================================
    def access_admin_panel(self):
        ...
        error_label = tk.Label(verify_window, text="", fg="#DC143C", font=("Arial", 9))
    ...
    def show_admin_panel(self):
        ...
        # Header
        header_frame = tk.Frame(admin_window, bg="#8B0000", height=60)
        header_frame.pack(fill="x")
        
        tk.Label(header_frame, text="ADMIN PANEL - USER MANAGEMENT & AUDIT",
                bg="#8B0000", fg="white", font=("Arial", 16, "bold")).pack(side="left", padx=20, pady=10)
        
        tk.Label(header_frame, text=f"Accessed by: {self.username}", 
                bg="#8B0000", fg="#FFB6C1", font=("Arial", 9)).pack(side="right", padx=20, pady=10)
        ...
        # Status bar
        status_frame = tk.Frame(admin_window, bg="#F5EFEF")
        status_frame.pack(fill="x")
        tk.Label(status_frame, text="Admin panel: View and export all user data",
                bg="#F5EFEF", font=("Arial", 9), fg="#666666").pack(anchor="w", padx=10, pady=5)
    ...
    def create_users_tab(self, parent):
        ...
        tk.Label(table_frame, text=f"Total Registered Users: {len(users)}", 
                font=("Arial", 11, "bold"), fg="#8B0000").pack(anchor="w", pady=(0, 10))
    ...
    def create_statistics_tab(self, parent):
        ...
        for i, (label, value) in enumerate(stat_data):
            ...
            label_widget = tk.Label(stats_frame, text=label, font=("Arial", 10, "bold"), fg="#8B0000")
